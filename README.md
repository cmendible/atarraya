...
1. When a deployment is pushed to Kubernetes, **atarraya-webhook** check for the ```atarraya/keyvault``` annotation to see if it needs to work its magic.
1. If the ```atarraya/keyvault``` annotation is present, **atarraya-webhook** proceeds as follows:
    ...
    1. Injects an Init Container named ```az-atarraya-init``` which only fucntion is to copy the **atarraya** executable into the ```atarraya-volume``` volume.
...
1. Then the container runs **atarraya** which does the following:
    ...
    1. If environment variables starting with ```ATARRAYA_SECRET_``` are found the executable strips the ```ATARRAYA_SECRET_``` prefix from the name of the variable and use this value to queries for secrets in the [Azure Key Vault](https://azure.microsoft.com/en-us/services/key-vault/) specified in the ```atarraya/keyvault``` annotation.
